...
const addDevMiddlewares = (app, options) => {
    ...
    app.use('/api', proxy({
        //TODO: YOUR SERVER
        target: 'http://dev.example.com/api',
        changeOrigin: true,
        pathRewrite: {
            '^/api': "" // remove path
        }
    ...
    app.get('/login', (req, res) => {
        const file = fs.readFileSync(path.join(compiler.outputPath, 'login.html'));
...
const addProdMiddlewares = (app, options) => {
    ...
    app.get('/login', (req, res) => res.sendFile(path.join(options.output.path, 'login.html')));
